...
// the leaderboard code is written by me
...
export default function SnakeHome({ user }: { user: User }) {
	...
	async function gameOver() {
		...
		const data = await submitScore(user.id, score);
	...
	async function submitScore(userId: string, score: number) {
		const data = await updateSnakeScore(userId, score).then(() => getSnakeLeaderboard(userId));
	...
	async function fetchLeaderboard(userId: string) {
		return await getSnakeLeaderboard(userId);
	}
	...
	useEffect(() => {
		fetchLeaderboard(user.id).then((data) => {
			setLeaderboard(data.leaderboard || []);
			setMe(data.me);
		});
	}, []);
