...
export function Chatbox({ recipient, user, roomId, type }: ChatboxProps) {
	...
	const fetchOlderMessages = async () => {
		if (!hasMore || !containerRef.current || isLoadingOldMsg || initialLoading || lastBatctLength.current < limit)
			return;
		...
		lastBatctLength.current = olderMessages.length;
		...
		<>
			...
			>
				<ChatProvider config={{ setMessages, messages, roomId, user, containerRef, isBlocked, isSystem }}>
					<div
						ref={containerRef}
						className="flex-1 h-full flex flex-col overflow-y-scroll py-4 px-1 pb-10 has-scroll-container relative custom-scrollbar"
					...
					>
						...
						<div ref={observerRef} className="h-10 absolute top-25 left-0 right-0 border-red-500 border"></div>
